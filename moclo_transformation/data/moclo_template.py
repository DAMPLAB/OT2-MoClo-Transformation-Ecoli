...
################################################################################
# Up to 88 rxns per single run and 24 rxns for triplicate of each combination of
# DNA assembly
...
################################################################################
# For this protocol, the biorad_96_wellplate_200ul_pcr, is placed on the top of the
# TempDeck alone without the Opentrons 96 well aluminum block.
################################################################################

# Load in Bio-Rad 96 Well Plate on temp deck for moclos, transformation, and
# outgrowth.
...
for i in range(12):
    p10_single.transfer(
        16,
        reaction_plate.wells(95).bottom(),
        reaction_plate.wells(i).bottom(0.3),
        new_tip='never',
    )
    p10_single.blow_out()
p10_single.drop_tip()
...
for i in range(12):
    p10_single.transfer(
        16,
        reaction_plate.wells(94).bottom(),
...
for i in range(12):
    p10_single.transfer(
        10,
        reaction_plate.wells(93).bottom(),
...
for i in range(12):
    p10_single.transfer(
        10,
        reaction_plate.wells(92).bottom(),
...
for i in range(12):
    p10_single.transfer(
        4,
        reaction_plate.wells(91).bottom(),
...
for i in range(12):
    p10_single.transfer(
        4,
        reaction_plate.wells(90).bottom(),
...
# This function checks the existance of DNA parts and returns for well location of
# the parts
...
# well location of output DNA combinations
...
# Seal the Reaction Plate with adhesive film, and incubate plate for 35 cycles of 37C
# for 1.5 minutes and 16C for 3 minutes, followed by 1 cycle at 50C for 5 minutes,
# and 80C for 10 minutes.

################################################################################
# Seal the Reaction Plate to avoid liquid evaporation.
# Discard the empty PCR tubes previously containing buffer, ligase, and restriction
# enzyme.
# Remove the Input_DNA_Plate from the Deck Space. Remaining DNA may be saved by
# sealing the Input_DNA_Plate with adhesive film and storing at -20°C
################################################################################
