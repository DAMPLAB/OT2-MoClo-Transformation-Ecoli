...
def get_config(config_path):
    ...
    if not config['output_folder_path']:
        messagebox.showinfo(
            ...
            "You will now select the folder to save the protocol and plate maps to. "
            "This can be changed later by editing settings.yaml in the "
...
# Max number of combinations if single is 88 (11 unique columns), and if triplicate is
# 24 (3 unique columns, each repeated 3 times)
def ask_single_or_triplicate():
    return str(input(
        "Enter 'single' if you want to run at most 88 single combinations or "
        "'triplicate' if you want to run at most 24 triplicate combinations: ")
    )
...
def check_number_of_combinations(combinations_limit, combinations_to_make):
    ...
    if combinations_limit == 'single':
        if number_of_combinations > 88:
            raise ValueError(
                'Too many combinations ({0}) requested. Max for single combinations '
                'is 88.'.format(number_of_combinations)
            )
    elif combinations_limit == 'triplicate':
        if number_of_combinations > 24:
            raise ValueError(
                'Too many combinations ({0}) requested. '
                'Max for triplicate combinations is 24.'.format(number_of_combinations)
            )
...
def create_protocol(
    ...
    with open(
            output_folder_path + '/' + 'moclo_transform_protocol.py', "w+"
    ) as protocol_file:
        # Paste in plate maps at top of file.
        protocol_file.write(
            'dna_plate_map_dict = ' + json.dumps(dna_plate_map_dict) + '\n\n'
        )
        protocol_file.write(
            'combinations_to_make = ' + json.dumps(combinations_to_make) + '\n\n'
        )
